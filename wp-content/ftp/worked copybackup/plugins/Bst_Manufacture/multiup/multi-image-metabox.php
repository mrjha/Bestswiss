...
function imagesliees($post){
	...
	foreach($imgaidrr as $k=>$i){
		...
		echo '<a onclick="addImage()" href="javascript:void(0);" class="multiimageupload" data-num="'.$no.'">'._x('Add New','file').'</a><a onclick="deletImage('.$meta.','.$no.')" href="javascript:void(0);" class="delbutton" data-num="'.$no.'">'.__('Delete').'</a>';
		echo '</div>';
		$z++;
		echo '</div>';
	}
                    ...
                    for (i = 0; i < objselection.length; i++) {
					    ...
					    if(i==0){
                           ...
                           jQuery('.gallerycontainer #droppable1').html(divcontain1);   
                            ...
                            j=i+1;
					    	divcontain = "<div id='droppable"+j+"' class=''><div class='cimage-entry'><input type='hidden' value='"+idval+"' data-num='"+j+"' class='id_img_c' id='image"+j+"' name='upimage[]'><div data-num='"+j+"' class='img-preview'><img height='100' width='100' alt='' src='"+idurl+"'></div><a onclick='addImage()' data-num='"+idval+"' class='multiimageupload' href='javascript:void(0);''>Add New</a><a data-num='"+j+"' class='delbutton' onclick='deletImage("+idval+","+j+")' href='javascript:void(0);'>Delete</a></div></div> ";
