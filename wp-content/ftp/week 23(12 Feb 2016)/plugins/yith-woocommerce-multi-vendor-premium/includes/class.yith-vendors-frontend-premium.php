...
if ( ! class_exists( 'YITH_Vendors_Frontend_Premium' ) ) {
    ...
    class YITH_Vendors_Frontend_Premium extends YITH_Vendors_Frontend {
        ...
        public function add_store_page_header() {
            if ( $this->is_vendor_page() ) {
                ...
                $args = array(
                    ...
                    'total_sales'        => count( $vendor->get_orders() ),
        ...
        public function enqueue_scripts() {
            ...
            if ( $show_report_abuse ) {
                $args = array(
                    ...
                    'messages' => array(
                        'success' => __( 'Abuse reported correctly.', 'yith_wc_product_vendors' ),
                        'empty'   => __( 'All fields are mandatory.', 'yith_wc_product_vendors' ),
                        'failed'  => __( 'Your request could not be processed. Please try again', 'yith_wc_product_vendors' )
                    ),
        ...
        public function get_registration_error_string( $field = 'all' ){
            $errors = array(
                'firstname'            => __( 'The First name field is mandatory', 'yith_wc_product_vendors' ),
                'lastname'             => __( 'The Last name field is mandatory', 'yith_wc_product_vendors' ),
                'location'             => __( 'The Store address field is mandatory', 'yith_wc_product_vendors' ),
                'email'                => __( 'The Email field is mandatory', 'yith_wc_product_vendors' ),
                'not_email'            => __( 'The Email address entered is not valid', 'yith_wc_product_vendors' ),
                'telephone'            => __( 'The Telephone field is mandatory', 'yith_wc_product_vendors' ),
                'vat_ssn'              => __( 'The VAT/SSN field is mandatory', 'yith_wc_product_vendors' ),
                'store_name'           => __( 'Insert the vendor name', 'yith_wc_product_vendors' ),
                'duplicated'           => __( 'A vendor with this name already exists', 'yith_wc_product_vendors' ),
                'terms_and_conditions' => __( 'Please, read and accept the terms & conditions', 'yith_wc_product_vendors' ),
                'antispam'             => __( 'Please, not spam here', 'yith_wc_product_vendors' ),
            );
        ...
        public function add_report_abuse_link() {
            ...
            $args = array(
                ...
                'submit_label' => apply_filters( 'yith_wpv_report_submit_button_label', __( 'Report', 'yith_wc_product_vendors' ) ),
                'vendor'       => $vendor,
        ...
        public function send_report_abuse() {
            ...
            if ( $check ) {
                ...
                $message = sprintf( __( "User %s (%s) is reporting an abuse on the following product: \n", 'yith_wc_product_vendors' ), $name, $from_email );
                $message .= sprintf( __( "Product details: %s (ID: #%s) \n", 'yith_wc_product_vendors' ), $product->post_title, $product->ID );

                if ( $vendor->is_valid() ) {
                    $message .= sprintf( __( "Vendor shop: %s (ID: #%s) \n", 'yith_wc_product_vendors' ), $vendor->name, $vendor->id );
                }

                $message .= sprintf( __( "Message: %s\n", 'yith_wc_product_vendors' ), $user_message );
                $message .= "\n\n\n";

                $message .= sprintf( __( "Product page: %s\n", 'yith_wc_product_vendors' ), get_the_permalink( $product->ID ) );

                if ( $vendor->is_valid() ) {
                    $message .= sprintf( __( "Vendor Account details: %s \n", 'yith_wc_product_vendors' ), $vendor->get_url( 'admin' ) );
                }
        ...
        public function woocommerce_product_meta() {
            ...
            <span class="item-sold">
                <?php _e( 'Item sold', 'yith_wc_product_vendors' ); ?>:
        ...
        public function add_sold_by_vendor( $product_title, $cart_item, $cart_item_key = false ) {
            ...
            if ( $vendor->is_valid() ) {
                $product_title .= ' (<small>' . _x( 'Sold by', 'Cart details: Product sold by', 'yith_wc_product_vendors' ) . ': ' . '<a href="' . $vendor->get_url( 'frontend' ) . '">' . $vendor->name . '</a></small>)';
            }
