	...
	private function query_report_data() {
        ...
        foreach ( $commission_ids as $commision_id ) {
            ...
            if( ! isset( $this->report_data->orders_count[ $order->id ] ) ) {
				...
				$orders_all_product[ $order->id ]					  = 0;
            }

			if( ! isset( $amount[ $order->id ] ) ){
				$amount[ $order->id ] = 0;
			}

			$amount[ $order->id ] = $amount[ $order->id ] + $commission->amount;
            ...
            foreach( $line_items as $line_item_id => $line_item ){
                if( in_array( $line_item['product_id'], $products ) ){
					...
					$orders_all_product[ $order->id ]       						 += absint( $line_item['qty'] );
                    $this->report_data->orders_all_product[ $order->id ]->order_date  = $order->post->post_date;
                }
            }
			$this->report_data->orders_all_product[ $order->id ]->count = $orders_all_product[ $order->id ];
        }
...
error_log( print_r( $this->report_data->orders_all_product, true ) );
		?>
