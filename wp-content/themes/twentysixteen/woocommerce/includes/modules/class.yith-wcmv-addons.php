...
class YITH_WCMV_Addons {
    ...
    public function get_plugin_landing_uri( $plugin_name, $context = 'uri' ){
        $plugin_link = '';
        if ( 'display' == $context ) {
            $coming_soon = ! empty( $this->plugins[$plugin_name]['coming_soon'] ) && $this->plugins[$plugin_name]['coming_soon'] ? sprintf( '<span class="add-ons-coming-soon">%s</span>', __( 'Coming Soon', 'yith_wc_product_vendors' ) ) : '';
            $plugin_link = $this->has_plugin( $plugin_name ) ? '' : sprintf( '<span class="yith-wcmv-required-plugin">(%s <a href="%s" target="_blank">%s</a> %s - %s %s %s).</span>%s',
                ...
                __( 'or greater', 'means: min version xxx or greater', 'yith_wc_product_vendors' ),
                $coming_soon
            );
        }
